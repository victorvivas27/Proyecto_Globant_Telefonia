package com.telefonia_vivas.constants;

public class ConstanteComuna {
    public static final String COMUNAS = "COMUNAS";
    public static final String NOMBRE_COMUNA = "NOMBRE_COMUNA";
    public static final String NOMBRE_EXISTE = "El nombre de la comuna ya existe";
    public static final String COMUNA_CREADA = "Comuna creada con exito";
    public static final String COMUNAS_LIST = "Lista de comunas";
    public static final String NOMBRE_COMUNA_NOT_NULL = "El nombre de la comuna no puede ser nulo";
    public static final String ID_COMUNA_NO_EXISTE = "El id de la comuna no existe: ";
    public static final String COMUNA_ENCONTRADA = "Comuna encontrada";
    public static final String COMUNA_ELIMINADA = "Comuna eliminada";
    public static final String COMUNA_MODIFICADA = "Comuna modificada";

}
